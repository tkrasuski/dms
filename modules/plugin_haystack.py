...
class SimpleBackend(object):
    regex = re.compile('[\w\-]{2}[\w\-]+', re.UNICODE)
    ignore = set(['i','lub','oraz','z','ze','do','od','u'])
    ...
    def after_insert(self,fields,id):
        ...
        for fieldname in self.fieldnames:
            words = set(self.regex.findall(fields[fieldname].decode('utf-8').lower())) - self.ignore
            for word in words:
                self.idx.insert(
                    fieldname = fieldname,
                    keyword = word,
    ...
    def meta_search(self,limit,mode,**fieldkeys):
        ...
        for fieldname in fieldkeys:
            if fieldname in self.fieldnames:
                words = set(self.regex.findall(fieldkeys[fieldname].decode('utf-8').lower()))
               # print 'words sa'
               # print words
               # print mode
...
class WhooshBackend(SimpleBackend):
    ...
    def indexes(self,*fieldnames):
        ...
        self.fieldnames = unicode(fieldnames)
        try:
        ...
        if ids:
            ...
            for id in ids:
                writer.update_document(id=unicode(id),
                                       **dict((name,unicode(fields[name]))
                                              for name in unicode(self.fieldnames) if unicode(name) in fields))
            writer.commit()
        return True
    def after_delete(self,queryset):
        if DEBUG: print 'after delete',queryset
    ...
    def meta_search(self,limit,mode,**fieldkeys):
        ...
        with self.ix.searcher() as searcher:
            for fieldname in fieldkeys:
                #print unicode(fieldkeys)
                parser = QueryParser(fieldname, schema=self.ix.schema)
                query = parser.parse(fieldkeys[fieldname])
                #print fieldname
